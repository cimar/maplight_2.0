...
public static List<String> getEntityTypeDropDown(){
	return find("SELECT DISTINCT RecipientState FROM CandidateContributions WHERE RecipientState is not null").fetch();
		
}	

public static List<String> getRecipientNamesDropdown(String state){
	return find("SELECT DISTINCT RecipientCandidateNameNormalized FROM CandidateContributions WHERE RecipientState = "+ state).fetch();
}
  
  public static List<String> getCandidatesNames() {
    return find("SELECT DISTINCT RecipientCandidateNameNormalized FROM CandidateContributions where RecipientCandidateNameNormalized is not null").fetch(); // CandidateContributions").fetch();
  }
...
//    }
    ...
    if (!recipient.isEmpty()) {
      if (recipient.equals("__anyone")) {
        ...
        data.append("c.RecipientCandidateStatus = Won"); //new Object[0]);"  //OK SO THIS REQUIRES ADDING A COLUMN TO OUR DATABASE FOR 'CURRENT' MEMBERS
